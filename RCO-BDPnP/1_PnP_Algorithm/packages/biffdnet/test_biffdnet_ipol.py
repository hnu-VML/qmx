"""
Denoise an image with the BiFFDNet denoising method
...
def biffdnet_imdenoiser(imnoisy, sigma, model=None, useGPU=True):
	r"""Denoises an input image (M x N) with BiFFDNet
	"""
	...
	if model is None:
		...
		# Create model
		print('Loading model ...\n')
		net = BiFFDNet(num_input_channels=in_ch)
		# Load saved weights
		if useGPU:
...
def biffdnet_vdenoiser(vnoisy, sigma, model=None, useGPU=True):
	r"""Denoises an input video (M x N x F) with BiFFDNet in a frame-wise manner
	"""
	if model is None:
		in_ch = 1
		model_fn = 'models/binet_gray.pth'
		# Absolute path to model file
		model_fn = os.path.join(os.path.abspath(os.path.dirname(__file__)), \
					model_fn)
	...
	# apply BiFFDNet denoising for each frame
	vshape = vnoisy.shape
...
def biffdnet_rgb_denoise(x, sigma,model):
...
def test_biffdnet(**args):
	r"""Denoises an input image with BiFFDNet
	"""
	# Init logger
	...
	if not args['dont_save_results']:
		...
		cv2.imwrite("biffdnet.png", outimg)
		if args['add_noise']:
			cv2.imwrite("noisy_diff.png", variable_to_cv2_image(diffnoise))
			cv2.imwrite("biffdnet_diff.png", variable_to_cv2_image(diffout))

if __name__ == "__main__":
	# Parse arguments
	parser = argparse.ArgumentParser(description="BiFFDNet_Test")
	...
	print("\n### Testing BiFFDNet model ###")
	print("> Parameters:")
	for p, v in zip(argspar.__dict__.keys(), argspar.__dict__.values()):
		print('\t{}: {}'.format(p, v))
	print('\n')

	test_biffdnet(**vars(argspar))
