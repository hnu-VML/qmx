...
from packages.biffdnet.models import BiFFDNet
...
def Reconstruction_single(args, dataname, method, sigma=[100/255, 50/255, 25/255, 12/255], 
    ...
    if method=="gap-tv":
        ...
        model.eval()
    elif method=="pnp-biffdnet":
        in_ch = 1
        model_fn = 'packages/biffdnet/models/binet_gray.pth'
        # Create model
        model = BiFFDNet(num_input_channels=in_ch).cuda()
        # Load saved weights
        state_dict = torch.load(model_fn)
        # model.load_state_dict(state_dict)
        model.load_state_dict({k.replace('module.',''): v for k,v in state_dict.items()})
...
if __name__ == "__main__":
    ...
    Reconstruction_single(argspar, "drop40_cacti.mat",
                          method="pnp-biffdnet",
                          sigma=[58/255,25/255,12/255,6/255],
                          iter_max=[10,10,10,10],
                          nframe=1)
    Reconstruction_single(argspar, "kobe32_cacti.mat",
                          method="pnp-biffdnet",
