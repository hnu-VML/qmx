#!/bin/sh
"""
Denoise all the sequences existent in a given folder using BiFastDVDnet.
...
from .models import BiFastDVDnet
from .bifastdvdnet import bifastdvdnet_seqdenoise, denoise_seq_bifastdvdnet
	...
	for idx in range(seq_len):
		...
		if len(suffix) == 0:
			out_name = os.path.join(save_dir,\
					('n{}_BiFastDVDnet_{}').format(sigmaval, idx) + fext)
		else:
			out_name = os.path.join(save_dir,\
					('n{}_BiFastDVDnet_{}_{}').format(sigmaval, suffix, idx) + fext)
...
def test_bifastdvdnet(**args):
	...
	model_temp = BiFastDVDnet(num_input_frames=NUM_IN_FR_EXT)
	...
	with torch.no_grad():
		...
		denframes = denoise_seq_bifastdvdnet(seq=seqn,\
										noise_std=noisestd,\
...
def bifastdvdnet_denoiser(vnoisy, sigma, model=None, useGPU=True, gray=False, MMCO=False):
	r"""Denoise an input video (H x W x F x C for color video, and H x W x F for
	     grayscale video) with BiFastDVDnet
	"""
	# start_time = time.time()
	...
	if model is None:
		model = BiFastDVDnet(num_input_frames=NUM_IN_FR_EXT, num_color_channels=nColor).cuda()
		# Load saved weights
		state_temp_dict = torch.load('./packages/bifastdvdnet/logs/simulation.pth')
	...
	with torch.no_grad():
		...
		outv = bifastdvdnet_seqdenoise( seq=vnoisy,\
									  noise_std=noisestd,\
									  windsize=NUM_IN_FR_EXT,\
									  model=model,\
               						  MMCO=MMCO)
	...
	# stop_time = time.time()
	# print('    BiFastDVDnet video denoising eclipsed in {:.3f}s.'.format(stop_time-start_time))
	return outv

if __name__ == "__main__":
	# Parse arguments
	parser = argparse.ArgumentParser(description="Denoise a sequence with BiFastDVDnet")
	...
	print("\n### Testing BiFastDVDnet model ###")
	print("> Parameters:")
	for p, v in zip(argspar.__dict__.keys(), argspar.__dict__.values()):
		print('\t{}: {}'.format(p, v))
	print('\n')

	test_bifastdvdnet(**vars(argspar))
